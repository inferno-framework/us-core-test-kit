...
require_relative 'observation_clinical_test/observation_clinical_test_validation_test'
require_relative 'observation_clinical_test/observation_clinical_test_must_support_test'
require_relative 'observation_clinical_test/observation_clinical_test_reference_resolution_test'

module USCoreTestKit
  module USCoreV501
    class ObservationClinicalTestGroup < Inferno::TestGroup
      title 'Observation Clinical Test Result Tests'
      short_description 'Verify support for the server capabilities required by the US Core Observation Clinical Test Result Profile.'
      description %(
  # Background

The US Core Observation Clinical Test Result sequence verifies that the system under test is
able to provide correct responses for Observation queries. These queries
must contain resources conforming to the US Core Observation Clinical Test Result Profile as
...
the [US Core Observation Clinical Test Result Profile](http://hl7.org/fhir/us/core/StructureDefinition/us-core-observation-clinical-test). Each element is checked against
...
read succeeds.
      ...
      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(YAML.load_file(File.join(__dir__, 'observation_clinical_test', 'metadata.yml'), aliases: true))
      end
      ...
      test from: :us_core_v501_observation_clinical_test_validation_test
      test from: :us_core_v501_observation_clinical_test_must_support_test
      test from: :us_core_v501_observation_clinical_test_reference_resolution_test
    end
  end
end
