...
the [Observation Heart Rate Profile](http://hl7.org/fhir/StructureDefinition/heartrate). Each element is checked against
...
read each reference found and will fail if any attempted read fails.
      ...
      id :us_core_v311_heartrate
      run_as_group

      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(YAML.load_file(File.join(__dir__, 'heartrate', 'metadata.yml')))
      end
  
      test from: :us_core_v311_heartrate_patient_code_search_test
      test from: :us_core_v311_heartrate_patient_category_date_search_test
      test from: :us_core_v311_heartrate_patient_category_status_search_test
      test from: :us_core_v311_heartrate_patient_code_date_search_test
      test from: :us_core_v311_heartrate_patient_category_search_test
      test from: :us_core_v311_heartrate_read_test
      test from: :us_core_v311_heartrate_provenance_revinclude_search_test
      test from: :us_core_v311_heartrate_validation_test
      test from: :us_core_v311_heartrate_must_support_test
      test from: :us_core_v311_heartrate_reference_resolution_test
    end
  end
end
