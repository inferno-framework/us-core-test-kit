...
must contain resources conforming to the Observation Body Weight Profile as
...
the [Observation Body Weight Profile](http://hl7.org/fhir/StructureDefinition/bodyweight). Each element is checked against
...
read succeeds.
      ...
      id :us_core_v311_bodyweight
      run_as_group

      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(YAML.load_file(File.join(__dir__, 'bodyweight', 'metadata.yml'), aliases: true))
      end
  
      test from: :us_core_v311_bodyweight_patient_code_search_test
      test from: :us_core_v311_bodyweight_patient_category_date_search_test
      test from: :us_core_v311_bodyweight_patient_category_status_search_test
      test from: :us_core_v311_bodyweight_patient_code_date_search_test
      test from: :us_core_v311_bodyweight_patient_category_search_test
      test from: :us_core_v311_bodyweight_read_test
      test from: :us_core_v311_bodyweight_provenance_revinclude_search_test
      test from: :us_core_v311_bodyweight_validation_test
      test from: :us_core_v311_bodyweight_must_support_test
      test from: :us_core_v311_bodyweight_reference_resolution_test
    end
  end
end
