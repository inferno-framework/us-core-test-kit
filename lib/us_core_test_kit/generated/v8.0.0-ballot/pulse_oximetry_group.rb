...
require_relative 'pulse_oximetry/pulse_oximetry_patient_category_lastupdated_search_test'
...
module USCoreTestKit
  module USCoreV800_BALLOT
    class PulseOximetryGroup < Inferno::TestGroup
      title 'Observation Pulse Oximetry Tests'
      short_description 'Verify support for the server capabilities required by the US Core Pulse Oximetry Profile.'
      description %(
  # Background

The US Core Observation Pulse Oximetry sequence verifies that the system under test is
...
read succeeds.
      ...
      id :us_core_v800_ballot_pulse_oximetry
      run_as_group

      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(YAML.load_file(File.join(__dir__, 'pulse_oximetry', 'metadata.yml'), aliases: true))
      end
  
      test from: :us_core_v800_ballot_pulse_oximetry_patient_code_search_test
      test from: :us_core_v800_ballot_pulse_oximetry_patient_code_date_search_test
      test from: :us_core_v800_ballot_pulse_oximetry_patient_category_date_search_test
      test from: :us_core_v800_ballot_pulse_oximetry_patient_category__lastUpdated_search_test
      test from: :us_core_v800_ballot_pulse_oximetry_patient_category_search_test
      test from: :us_core_v800_ballot_pulse_oximetry_patient_category_status_search_test
      test from: :us_core_v800_ballot_pulse_oximetry_read_test
      test from: :us_core_v800_ballot_pulse_oximetry_provenance_revinclude_search_test
      test from: :us_core_v800_ballot_pulse_oximetry_validation_test
      test from: :us_core_v800_ballot_pulse_oximetry_must_support_test
      test from: :us_core_v800_ballot_pulse_oximetry_reference_resolution_test
    end
  end
end
